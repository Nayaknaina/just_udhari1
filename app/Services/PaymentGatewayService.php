...
use App\Services\PaymentGateways\Razorpay;
class PaymentGatewayService
{
    ...
    public function __construct($gateway_id = null)
    {
        ...
        if(!empty($gatewaysetting)){
            //$class_name = ucfirst(strtolower(str_replace("_","",str_replace(" ","",$gatewaysetting->gateway_name))));
			$class_name = ucfirst(strtolower(str_replace("_","",str_replace(" ","",$gatewaysetting->gateway_name))));
            $gatewaytype =  "App\Services\PaymentGateways\\{$class_name}";
    ...
    public function initiatePayment($order=null,$callback="")
    {
        $gateway = $this->getGateway();
        return $gateway->initiatePayment($order,$callback);
    }
