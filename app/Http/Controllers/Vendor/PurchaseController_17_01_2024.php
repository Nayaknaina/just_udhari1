     ...
     */
     ...
     public function store(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            DB::beginTransaction();
            try{
                $stock_type = ($request->stocktype!='loose')?$request->stocktype:'other';
                $purchase = Purchase::create([
                    'supplier_id' => $request->supplier,
                    'bill_no' => $request->bill_no,
                    'bill_date' => $request->bill_date,
                ...
                if($stock_type=='artificial'){
                    ...
                    foreach($request->metals as $bind=>$metal){
                        foreach($request->product_name[$bind] as $itmk=>$item){
                            ...
                            $input_arr['name'] = $item;
                            $input_arr['quantity'] =($stock_type=='other')?0:1;
                    ...
                    foreach($request->metals as $bind=>$metal){
                        ...
                        if(!empty($request->product_name[$bind])){
                            foreach($request->product_name[$bind] as $itmk=>$item){
                                ...
                                $input_arr['branch_id'] = auth()->user()->branch_id;
                                $input_arr['shop_id'] = auth()->user()->shop_id;
                                $input_arr['item_type'] = ($request->stocktype=='loose')?'other':$request->stocktype;
                                // echo "<pre>";
                                // print_r($input_arr);
                                // echo "<pre>";
                                $newCategoryIds = [];
