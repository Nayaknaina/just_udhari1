...
class EnrollCustomerController extends Controller {
     ...
     */
     ...
     public function index(Request $request) {
        $sch_query = ShopScheme::with('schemes')->where('ss_status',1) ;
        Shopwhere($sch_query) ;
    ...
    public function addcustomer(Request $request){
        ...
        if ($validator->fails()) {
            ...
            if(empty($exist)){
                $valid = true;
                $input_arr = [
                    "custo_unique"=>uniqid() . time(),
                    "shop_id"=>$shop,
                    "branch_id"=>$branch,
                    "custo_full_name"=>$request->name,
                    "custo_fone"=>$request->fone,
                    "fone_varify"=>1,
                    "custo_address"=>$request->addr,
                ];
