...
class EcommCmsController extends Controller {
    ...
    public function saveslider(Request $request){
        ...
        $slide_count = EcommSlider::where('shop_id',auth()->user()->shop_id)->count() ;
        if($slide_count < 5){
...
///---------------------------TERM CONTENT-----------------------------------------------------------//
	...
	public function termstatus(){
        $ecommtermobj = new EcommTerm;
        $existterm = $ecommtermobj->content(app('userd')->shop_id) ;
        $existterm->term_status = ($existterm->term_status==0)?'1':'0';
        $bool = ($existterm->update())?true:false;
        return response()->json(['status'=>$bool]);
    }
///---------------------------END TERM CONTENT------------------------------------------------//
...
///---------------------------POLICY CONTENT------------------------------------------------//
    ...
    public function savepolicy(Request $request){
        ...
        if ($validator->fails()) {
            ...
            if(empty($existpolicy)){
                $input['shop_id'] = app('userd')->shop_id;
                $input['branch_id'] = app('userd')->banch_id;
	...
	public function privacystatus(){
        $ecommtermobj = new EcommPrivacy;
        $existpolicy = $ecommtermobj->content(app('userd')->shop_id) ;
        $existpolicy->policy_status = ($existpolicy->policy_status==0)?'1':'0';
        $bool = ($existpolicy->update())?true:false;
        return response()->json(['status'=>$bool]);
    }
...
///---------------------------END POLICY CONTENT------------------------------------------------//
    ...
    public function refundpolicy(){
        ...
        $ecommrefundobj = new EcommRefund;
        $existrefund = $ecommrefundobj->content(app('userd')->shop_id);
        return view("vendors.ecommerce.sitecontent.cmsrefund",compact('existrefund'));
    ...
    public function saverefundpolicy(Request $request){
        ...
        if ($validator->fails()) {
            ...
            if(empty($existrefund)){
                $input['shop_id'] = app('userd')->shop_id;
                $input['branch_id'] = app('userd')->banch_id;
                $input['refp_unique'] = uniqid() . time();
            }

            $input['refp_content'] = $request->info;
            $input['refp_status']='0';
            $currrefund = (!empty($existrefund))?$existrefund->update($input):EcommRefund::create($input);
            if($currrefund){
                return response()->json(["valid"=>true,"status"=>true, 'msg' => "Refund Policy Info Succesfully Saved !"]);
            }else{
                return response()->json(["valid"=>true,"status"=>false, 'msg'=>"Refund Policy Info Saving Failed !"]);
            }
        }
    }

	public function refundstatus(){
        $ecommtermobj = new EcommRefund;
        $existpolicy = $ecommtermobj->content(app('userd')->shop_id) ;
        $existpolicy->refp_status = ($existpolicy->refp_status==0)?'1':'0';
        $bool = ($existpolicy->update())?true:false;
        return response()->json(['status'=>$bool]);
    }
...
///---------------------------DESCLAIMER CONTENT------------------------------------------------//
	...
	public function desclaimerstatus(){
        $ecommdescobj = new EcommDesclaimer;
        $existdesc = $ecommdescobj->content(app('userd')->shop_id) ;
        $existdesc->desc_status = ($existdesc->desc_status==0)?'1':'0';
        $bool = ($existdesc->update())?true:false;
        return response()->json(['status'=>$bool]);
    }
///---------------------------END DESCLAIMER CONTENT------------------------------------------------//
    ...
    public function shiping(){
        ...
        $ecommdescobj = new EcommShiping;
        $existdesc = $ecommdescobj->content(app('userd')->shop_id);
        return view("vendors.ecommerce.sitecontent.cmsship",compact('existdesc'));

    }

    public function saveshiping(Request $request){
        $ecommdescobj = new EcommShiping;
        $existdesc = $ecommdescobj->content(app('userd')->shop_id);
        $validator = Validator::make(
            $request->all(),
        ...
        if ($validator->fails()) {
            ...
            $input['policy_content'] = $request->info;
            $input['policy_status']='0';
            $currdesc = (!empty($existdesc))?$existdesc->update($input):EcommShiping::create($input);
            if($currdesc){
                return response()->json(["valid"=>true,"status"=>true, 'msg' => "Shiping Policy Succesfully Saved !"]);
            }else{
                return response()->json(["valid"=>true,"status"=>false, 'msg'=>"Shiping Policy Saving Failed !"]);
            }
        }
    }
	public function shipingstatus(){
        $ecommshipobj = new EcommShiping;
        $existship = $ecommshipobj->content(app('userd')->shop_id) ;
        $existship->policy_status = ($existship->policy_status==0)?'1':'0';
        $bool = ($existship->update())?true:false;
        return response()->json(['status'=>$bool]);
    }
	///---------------------------END SHIPPING CONTENT------------------------------------------------//
	///---------------------------USER (CUSTOMER) ACCOUNT  CONTENT------------------------------------------------//

	 public function acdelete(){

		$ecommaccobj = new EcommDelete;
		$existdesc = $ecommaccobj->content(app('userd')->shop_id);
		return view("vendors.ecommerce.sitecontent.cmsacdelete",compact('existdesc'));

	}

	public function savesacdelete(Request $request){
		$ecommacobj = new EcommDelete;
		$existdesc = $ecommacobj->content(app('userd')->shop_id);
		$validator = Validator::make(
			$request->all(),
			[
				"info" => "required|string",
		...
		if ($validator->fails()) {
			...
			$input['policy_content'] = $request->info;
			$input['policy_status']='0';
			$currdesc = (!empty($existdesc))?$existdesc->update($input):EcommDelete::create($input);
			if($currdesc){
				return response()->json(["valid"=>true,"status"=>true, 'msg' => "Account Deletion Policy Succesfully Saved !"]);
			}else{
				return response()->json(["valid"=>true,"status"=>false, 'msg'=>"Account Deletion Policy Saving Failed !"]);
			}
		}
	}

	public function acdeletestatus(){
		$ecommacobj = new EcommDelete;
		$existplc = $ecommacobj->content(app('userd')->shop_id) ;
		$existplc->policy_status = ($existplc->policy_status==0)?'1':'0';
		$bool = ($existplc->update())?true:false;
		return response()->json(['status'=>$bool]);
	}
	///---------------------------END USER (CUSTOMER)   CONTENT------------------------------------------------//
}
