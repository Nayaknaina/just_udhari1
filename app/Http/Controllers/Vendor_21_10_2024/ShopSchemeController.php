...
class ShopSchemeController extends Controller {
    ...
    //     $enquiries = SchemeEnquiry::with("scheme","customer")->get();
    //     //dd($enquiries);
    ...
    public function manualpay(Request $request){
        ...
        $currentPage = $request->input('page', 1);
        $active_schemes = ShopScheme::where(['shop_id'=>auth()->user()->shop_id,'ss_status'=>1])->pluck('id');
        $customers = EnrollCustomer::with('schemes','info','groups','emipaid')->whereIn('scheme_id',$active_schemes)->paginate($perPage, ['*'], 'page', $currentPage);
        //$schemes = $query->paginate($perPage, ['*'], 'page', $currentPage);
        if ($request->ajax()) {
