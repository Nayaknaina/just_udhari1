...
$data = new_component_array('newbreadcrumb',"Scheme Enroll New",$path) 
@endphp 
        ...
        <div class = "row justify-content-center">
            ...
            <div class="row">
                ...
                <div class="col-lg-4 form-group ">
                    ...
                    <select name="customer_id" id = "customer_id" class="form-control select2 h-auto">
                ...
                <div class="col-lg-12 form-group " id="customer_choice_container">
                    <div id="customer_names_container" class="row">
						...
						<div class="col-lg-3 form-group">
							<label for="pai_medium">Pay Medium</label>
							<input type="hidden" class="pay_to" name="pay_to[]" value="0" id="pay_to">
							<select class="form-control pay_medium" id="pay_medium" name="pay_medium[]" disabled >
                                <option value="">SELECT</option>
                                @php 
                                    $medium_arr = [
						...
						<div class="col-lg-2 form-group">
                            ...
                            <input type="date" id="enroll_date" name="enroll_date[]" class="form-control form-group enroll_date text-center px-1" placeholder="Enter Enroll Date" required value="{{ date('Y-m-d',strtotime("now")); }}">
                        </div>
						<div class="col-lg-2 form-group">
                            <label for="operation" id="operation_label" class="operation_label">Operation</label>
                            <select name="operation[]" id="operation" class="form-control operation text-center px-1" style="font-weight:bold;">
                                <option value="pay">With Emi Pay</option>
                                <option value="enroll">Enroll Only</option>
                            </select>
                        </div>
                        <div class="col-md-5 form-group enroll_emi_pay" style="box-shadow:1px 2px 3px 4px #cacadb;">
                            <div class="row">
                                <div class="col-lg-7">
                                    <label for="pay_emi" id="pay_emi_label" class="pay_emi_label">Amount</label>
                                    <div class="input-group">
                                        <input type="number" id="pay_emi" name="pay_emi[]" class="form-control pay_emi" placeholder="EMI Amount">
                                        <span class="input-group-text p-0 px-1" id="basic-addon2" style="border-radius:0 5px 5px 0"><b>Rs.</b></span>
                                    </div>
                                </div>
                                <div class="col-lg-5">
                                    <label for="pay_date" id="pay_date_label" class="pay_date_label">Payment Date</label>
                                    <input type="date" id="pay_date" name="pay_date[]" class="form-control form-group enroll_date text-center px-1" placeholder="Enter Enroll Date"  value="{{ date('Y-m-d',strtotime("now")); }}">
                                </div>
                            </div>
                        </div>
                    </div>
  ...
  <script>
    ...
    $(document).ready(function() {
		...
		$(document).on('change',".operation",function(e){
            var index = $('.operation').index(this);
            if($(this).val()=='pay'){
                $('.enroll_emi_pay').eq(index).show();
            }else{
                $('.enroll_emi_pay').eq(index).hide();
            }
        });
    ...
    $(document).ready(function() {
		...
		$(document).on("input change",".token_amt",function(){
            ...
            if($(this).val()!="" && $(this).val()>0){
				...
				$(".pay_to").eq(my_index).val('0');
                sel_ele.val('');
            }
			const token = parseInt($(this).val());
            const emi =  parseInt($('.emi_amt').eq(my_index).val());
            var paid = 0;
            if(token > emi){
                paid = emi;
            }else{
            ...
            $('.pay_emi').eq(my_index).val(paid);
        });
        
		 $(document).on('change','.pay_medium',function(){
            var my_index = $('.pay_medium').index(this);
            //alert(my_index);
            //var sel_ele = $(".pay_medium").eq(my_index);
            $(".pay_to").eq(my_index).val($(this).find('option:selected').data('value'));
        });
    ...
    $('#customer_multiple').on('input',function(){
        ...
        for (var i = 2; i <= multiple; i++) {
			...
			ele.find('.pay_to').val(0);
            ele.find('.operation').val('enroll');
			$($(document).find('.enroll_emi_pay').eq(i-1)).hide();
        }
    })
