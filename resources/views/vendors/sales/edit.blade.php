...
@php 
$anchor = ['<a href="'.route('sells.show',$sell->id).'" style="float:right;" class="btn btn-sm bg-light border-dark text-dark"><i class="fa fa-eye"></i> View</a>','<a href="'.route('sells.index').'" class="btn btn-sm btn-outline-info"><i class="fa fa-list"></i> All</a>'];
$path = ["Sell Bills"=>route('sells.index')];
$data = new_component_array('newbreadcrumb',"Edit Sell Bill",$path) 
@endphp 
                      ...
                      @method('put')
						<div class="row mb-2" style="border-bottom:1px solid #a6a6a6;">
                            ...
                            <div class="input-group mb-1 col-md-2 p-0">
                                ...
                                <input type="text" name="bill_num" id="bill_num" class="form-control text-center px-1" placeholder="Bill Number" value="{{ $sell->bill_no }}">
                            </div>
                            <div class="input-group mb-1 col-md-2 p-0">
                                <div class="input-group-prepend">
                                    <label for="bill_num" class="input-group-text mb-0 px-1">Date</label>
                                </div>
                                <input type="date" name="bill_date" id="bill_date" class="form-control text-center px-1" placeholder="Bill Ddate" value="{{ $sell->bill_date }}" required>
                            </div>
                            <div class="input-group mb-1 col-md-2 p-0">
                                <div class="input-group-prepend">
                                    <label for="vndr_gst" class="input-group-text mb-0 px-1">GSTIN</label>
                                </div>
                                <input type="text" name="vndr_gst" id="vndr_gst" class="form-control text-center px-1" placeholder="GST Number" value="{{ $sell->bill_gst }}" required="" readonly="">
                            </div>
                            <div class="input-group mb-1 col-md-2 p-0">
                                <div class="input-group-prepend">
                        ...
                        <div id="custo_info_row" class="row mb-2  p-2" >
                            @php  
                                $gst_apply = ($sell->gst_apply==1)?'gst_yes':'gst_no'; 
                                $$gst_apply = 'checked';
                            @endphp
							<label  class="bg-light h-auto px-1 mb-1 col-md-2 p-0" style="box-shadow: 1px 2px 3px black;border:1px dashed blue;">
                                <label for="gst_yes" class="text-success mb-0 gst_set_label {{ @$gst_yes }}" >
                                    <input type="radio" name="gst_apply" value="yes" id="gst_yes" class="" required {{ @$gst_yes }}> GST Bill
                                </label>
                                <label for="gst_no" class="text-danger mb-0 gst_set_label {{ @$gst_no }}" >
                                    <input type="radio" name="gst_apply" value="no" id="gst_no" class="" required  {{ @$gst_no }}> Rough Estimate
                                </label>
                            </label>
                            <label for="to" class="col-md-2 p-0">
                                @php  
                                    $bill_to = ($sell->sell_to=='R')?'retail':'whole'; 
                                    $$bill_to = 'selected';
                                @endphp
                                <div class="input-group">
                                    <select class="form-control text-center" name="to" style="font-weight:bold;color:#044fdd;" required id="to">
                                        <option value="">Category ?</option>
                                        <option value="R" {{ @$retail }}>Retail</option>
                                        <option value="W" {{ @$whole }} >Whole Sell</option>
                                    </select>
                            ...
                            <div class="form-group col-md-4 p-0 mb-1">
								<input type="hidden" name="custo_id" value="{{ $sell->custo_id }}"  id="is_exist">
                                <!--<input type="hidden" name="exist" value="yes" disabled id="is_exist">-->
                                <input type="text" name="custo_name" id="custo_name" class="form-control" placeholder="Name"  required value="{{ $sell->custo_name }}" readonly>
                                
                                <ul class="col-12" id="custo_list" style="display:none;"></ul>
                            </div>
                            <div class="form-group col-md-2 p-0  mb-1">
                                <input type="text" name="custo_mobile" id="custo_mobile" class="form-control" placeholder="Mobile"  oninput="digitonly(event,10);" onchange="digitonly(event,10);" required value="{{ $sell->custo_mobile }}" readonly>
                            </div>
                            <div class="form-group col-md-2 p-0  mb-1">
                                <div class="input-group">
                                    <div class="input-group-prepend">
                                    ...
                                    @endphp
                                    <label id="scheme_balance" class="text-center text-info form-control px-1 text-center" style="border-radius:0 15px 15px 0;overflow:hidden;">{{ (@$sell->customer->schemebalance->remains_balance+$scheme_pay_sum)??'NA' }}</label>
                                </div>
                            </div>
                        </div>
                        <div class="row mb-2" style="border-bottom:1px solid #a6a6a6;">
                            <div class="table-responsive">
                                <table id="CsTable" class="table table_theme table-bordered table-stripped dataTable">
                                    ...
                                    @php 
                                        $mk_readonly = strtolower($sell->sell_to);
                                        $$mk_readonly = 'readonly';
                                    @endphp 
                                    <tbody id="item_body">
                                        ...
                                        <tr id="item_{{ $ik }}" class="item_row">
                                            ...
                                            <td>
                                                ...
                                                @php 
                                                    $caret_perc = ($item->item_caret!=0 && $item->item_caret!="")?(($item->item_caret*100)/24):false;
                                                    $fine = ($caret_perc)?($item->item_quantity*$caret_perc)/100:false;;
                                                @endphp
                                                <div class="col-12 p-0">
                                                    <input type="hidden" name="type[]" value="{{ ucfirst($item->stock->item_type) }}">
                                                    <input type="hidden" name="caret[]" value="{{ ($caret_perc)?$item->item_caret:"" }}">
                                                    <input type="hidden" name="id[]" value="{{ $item->stock_id }}">
                                                    <input type="hidden" name="source[]" value="{{ $item->place_source }}">
                                                    <input type="hidden" name="item_id[]" value="{{ $item->id }}">
                                                    <input type="text" name="item_name[]" class="form-control w-100 item_info  px-1 readonly" placeholder="Item Name" value="{{ $item->item_name }}" {{ ($ik==0)?'required':'' }} readonly>
                                                    <div class="pre_info_div" >
                                                        <h5 class="info_block curr_cate text-center">{{ ucfirst($item->stock->item_type) }}</h5>
                                                        ...
                                                        <div class="input-group input-group-sm">
                                                            ...
                                                            <input type="text" class="form-control info_block text-right" aria-label="Small" name="curr_rate" disabled value="{{ $item->stock->rate }}">
                                                        ...
                                                        <div class="input-group input-group-sm">
                                                            ...
                                                            <input type="text" class="form-control info_block  text-right" aria-label="Small"  name="curr_make" disabled  value="{{ $item->stock->labour_charge }}">
                                                            <span class="input-group-text info_block" id="basic-addon2">Rs.</span>
                                                        </div>
                                                        <div class="input-group input-group-sm">
                                                            <div class="input-group-prepend">
                                                            ...
                                                            <input type="text" class="form-control info_block  text-right" aria-label="Small" name="curr_avail" disabled  value="{{ $item->stock->available }}">
                                                        </div>
                                                        <div class="input-group input-group-sm">
                                                            <div class="input-group-prepend">
                                                                <span class="input-group-text info_block label" id="inputGroup-sizing-sm">CARET </span>
                                                            </div>
                                                            <input type="text" class="form-control info_block  text-right" aria-label="Small" name="curr_caret" disabled  value="{{ ($caret_perc)?$item->item_caret."K (".number_format($caret_perc,2,'.','')." %)":'----' }}">
                                                        </div>
                                                        <div class="input-group input-group-sm">
                                                            <div class="input-group-prepend">
                                                                <span class="input-group-text info_block label" id="inputGroup-sizing-sm">FINE </span>
                                                            </div>
                                                            <input type="text" class="form-control info_block text-right" aria-label="Small" name="curr_fine" disabled value="{{ ($fine)?number_format($fine,2,'.',''):'----' }}">
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td style="position:relative;">
                                                <input type="text" name="item_quant[]" class="form-control w-100 item_info  text-right  px-1 readonly" placeholder="Quantity" oninput="calculatetotal()" value="{{ $item->item_quantity }}" {{ ($ik==0)?'required':'' }} readonly >
                                                <span class="unit {{ ($item->stock->item_type!='artificial')?'active':'deactive' }}">Grm</span>
                                            </td>
                                            @php 
                                                $lbr_readonly = ($item->stock->item_type=='artificial')?'readonly':'';
                                            @endphp
                                            <td>
                                                <input type="text" name="chrg_perc[]" class="form-control w-100 item_info  text-right  px-1  lbr_chrg_perc {{ @$r }} {{ $lbr_readonly }}" placeholder="Charge %"  oninput="togglecharge($(this),'item_chrg');calculatetotal()" value="{{  $item->labour_perc }}" data-target="lbr_chrg_rs" {{ @$r }} {{ $lbr_readonly }}>
                                            </td>
                                            <td>
                                                <input type="text" name="item_chrg[]" class="form-control w-100 item_info  text-right  px-1  lbr_chrg_rs {{ @$w }} {{ $lbr_readonly }}" placeholder="Charge Rs." oninput="togglecharge($(this),'chrg_perc');calculatetotal()"  value="{{  $item->labour_charge }}" data-target="lbr_chrg_perc" {{ @$w }} {{ $lbr_readonly }}>
                                            </td>
                                            
                                            <td class="bg-light text-dark">
                                                <input type="text" name="now_rate[]" class="form-control item_info text-center px-1" value="{{ $item->item_rate }}" id=""  placeholder="Applicable" oninput="lbrconverttoggle($(this));calculatetotal()" {{ ($ik==0)?'required':'' }}>
                                        ...
                                        @if(!empty($item->elements))
                                            ...
                                            <tr class="element_{{ $ik }} element_row">
                                                <td class="text-right">E &#10148;</td>
                                                <td>
                                                    <input type="text" name="element[{{ $ik }}][name][]" class="form-control ele_content readonly text-center" readonly value="{{ $ele['name'] }}">
                                                </td>
                                                <td colspan="2" class="ele_caret">
                                                    <input type="text" name="element[{{ $ik }}][caret][]" class="form-control ele_content readonly text-center" readonly value="{{ $ele['caret'] }}">
                                                </td>
                                                <td colspan="2" class="ele_count">
                                                    <input type="text" name="element[{{ $ik }}][quant][]" class="form-control ele_content readonly text-center" readonly value="{{ $ele['quant'] }}">
                                                </td>
                                                <td>
                                                <input type="text" name="element[{{ $ik }}][cost][]"  class="form-control w-100 ele_content  text-right  px-1 element_total readonly" placeholder="Total" readonly="" required="" value="{{ $ele['cost'] }}">
                                                </td>
                                            </tr>
                        ...
                        <div class="row mb-2" style="border-bottom:1px solid #a6a6a6;">
                            ...
                            <div class="col-md-3 col-12  p-1">
                                <div class="input-group mb-3"  id="rs_unit">
									...
									<input type="text" name="total_disc_amnt" class="form-control readonly text-right" value="{{ $sell->disc_amnt??0 }}" id="total_disc_amnt" required style="font-weight:bold;width:35%;padding-right:20px;" onfocus="$(this).select()" oninput="convertpercentage()" >
                                </div>
                            </div>
                            <div class="col-md-2 col-12  p-1 {{ ($sell->gst_apply==0)?'deactive':''; }}" id="gst_block">
...
<script>
  ...
  $(document).ready(function() {
    ...
    $('.delete_item').change(function(){
        ...
        if($(this).is(':checked')){
            $('#item_body > tr.item_row').eq(num).addClass('row-disabled');
            $('#item_body > tr.item_row').eq(num).find('td').addClass('table-disabled');
            $('tr.element_'+num).addClass('row-disabled');
            $('tr.element_'+num+'>td').addClass('table-disabled');
            $("#delete_item_count").text(+$("#delete_item_count").text()+ +1);
            $('.pre_info').eq(num).hide();
        }else{
            $('#item_body > tr.item_row').eq(num).removeClass('row-disabled');
            $('#item_body > tr.item_row').eq(num).find('td').removeClass('table-disabled');
            $('tr.element_'+num).removeClass('row-disabled');
            $('tr.element_'+num+'>td').removeClass('table-disabled');
            $("#delete_item_count").text(+$("#delete_item_count").text()- +1);
        }
        $('div.pre_info_div').eq(num).hide();
    ...
    $("#add_rows").click(function(e){
        ...
        tr.find('td').each(function(i,v){
			...
			$(tr.find('td>div>div.pre_info_div')).hide();
            $(tr.find('td>[name="chrg_perc[]"],td>[name="item_chrg[]"]')).removeClass('readonly').prop('readonly',false);
        });
    ...
    function getitem(input,item_request){
        ...
        $.get("{{ route('sells.item') }}","item="+input.val(),function(response){
			...
			input.next('div.pre_info_div').hide();
            $('#item_body>tr').eq(ind).find('td').each(function(i,td){
                $(this).find('input').val('');
                if($(this).find('input').attr('name')!="item_total[]"){
                    $(this).find('input').removeClass('readonly');
    ...
    $(document).on('click','.item_target',function(e){
        ...
        if(item_data.element && item_data.element.length>0){
            ...
            $.each(elements,function(ei,ele){
                console.log(ele);
                tr+='<tr class="element_'+ind+' element_row">';
    ...
    function removerow(self){
        const tr_ind = $($(document).find('tbody>tr.item_row')).index(self.parent('td').parent('tr.item_row'));
        $('tbody>tr.item_row').eq(tr_ind).remove();
        $(document).find('tbody>tr.element_'+tr_ind).remove();
        const trs = $('tbody>tr.item_row');
    ...
    function calculatetotal(disc_amnt_req = true){
        ...
        if(go_ahead){
            $('tbody>tr.item_row').each(function(i,v){
                ...
                if(!$(this).hasClass('row-disabled')){
                    if($('[name="item_name[]"]').eq(i).val()!=""){
                        ...
                        var chrg_perc = $('[name="chrg_perc[]').eq(i).val()??0;
                        var item_chrg = $('[name="item_chrg[]"]').eq(i).val()??0;
                        var cost = 0;
                        if($('[name="type[]"]').val()!='Artificial'){
                            if(chrg_perc!=0 && ($('[name="to"]').val()!="" && $('[name="to"]').val()=="W")){
                                item_chrg = 0;
                                var ttl_perc = +((($('[name="caret[]"]').eq(i).val()*100)/24).toFixed(2)) + +chrg_perc;
                                cost = (((item_qunt*ttl_perc)/100)*apply_rate).toFixed(2);
                            }else{
                                if(item_chrg!=0 &&  ($('[name="to"]').val()!="" && $('[name="to"]').val()=="R")){
                                    chrg_perc = 0;
                                    cost = ((+apply_rate * +item_qunt)+ +(item_qunt*item_chrg)).toFixed(2);
                                }
                            }
                        }else{
            ...
            $($(document).find('tbody>tr.element_row')).each(function(elkey,ele){
                if(!$(this).hasClass('row-disabled')){
                    sum+= +$(this).find('td>.element_total').val();
                }
            })
            ...
            var disc_val = ((new_sub*disc)/100)??0;
            if(disc_amnt_req){
