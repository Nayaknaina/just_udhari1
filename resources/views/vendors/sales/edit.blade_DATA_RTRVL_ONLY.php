                  ...
                  <form id = "submitForm" method="POST" action="{{ route('sells.update',$sell->id)}}" class = "myForm" enctype="">
                      ...
                      <div class="row mb-2" style="border-bottom:1px solid #a6a6a6;">
                        ...
                        @php 
                            ...
                            $Scheme = $Cash = "off";
                        @endphp
                      @if($sell->payments->count()>0)
                        @foreach($sell->payments as $pi=>$pay) 
                            @if($pi==0)
...
<script>
  ...
  $(document).ready(function() {
    ...
    //   }
        $(this).prev('label').toggleClass('btn-default  btn-danger');
        if($(this).is(':checked')){
            $("#remain").val(+rmns + +amnt);
            $('.pay_col').addClass('pay_col_disable').removeClass('pay_col');
        }else{
            $("#remain").val(+rmns - +amnt);
            $('.pay_col_disable').addClass('pay_col').removeClass('pay_col_disable');
        }
        // calculatetotal();
        // validalance();
    });
  ...
  $("#more_pay_done").click(function(e){
      ...
      if(opt_count > 0){
          $(document).find(".more_added_pay").each(function(i,v){
              ...
              html+=      '</div>';
              html+=      '<input type="text" class="form-control" aria-label="Small" aria-describedby="inputGroup-sizing-sm" name="amount[]" value="'+amount+'" readonly style="font-weight:bold;">';
              html+= '</div></div>';
