...
<section class = "content">
    <div class = "container-fluid">
        ...
        @foreach($purchase->stocks as $key=>$stock)
            ...
            @endif
            @if($num==0)
                <div class="row main_bill_block stock_block my-2">
                    ...
                    <div class="col-lg-3">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control rate" id="rate" name="rate[]" min="1" step="any" placeholder="Enter Rate" value="{{  $stock->rate  }}" >
                        </div>
                    </div>
            ...
            @if($num<=$count)
                <tr id="main_item_tr_1" class="main_item_tr">
                    <td class="sn-box">
                        <input type="hidden" name="stock_id[{{ $input_num }}][]" value="{{ $stock->id }}" id="stock_id_bndl">
                    ...
                    <td>
                        <input type="text" class="tb_input" name="product_name[{{ $input_num }}][]" id="product_name_{{ $loop->index + 1 }}" placeholder="Product Name" value="{{ $stock->name }}" >
                    </td>
                    <td>
                        <input type="number" class="tb_input" name="quantity[{{ $input_num }}][]" id="quantity_{{ $loop->index + 1 }}" placeholder="Quantity" oninput="calculate(this)" value="{{ $stock->quantity }}"   min = "0" step = "any" readonly style="width:50px;">
                    </td>
                    <td>
                        <input type="number" class="tb_input" name="carat[{{ $input_num }}][]" id="carat_{{ $loop->index + 1 }}" placeholder="Carat" oninput="calculate(this)" value="{{ $stock->carat }}"   min = "0" step = "any"  style="width:70px;">
                    </td>
                    <td>
                        <input type="number" class="tb_input" name="gross_weight[{{ $input_num }}][]" id="grossWeight_{{ $loop->index + 1 }}" placeholder="Gross Weight" oninput="calculate(this)" value="{{ $stock->gross_weight }}"   min = "0" step = "any"  style="width:80px;" >
                    </td>
                    <td>
                        <input type="number" class="tb_input" name="net_weight[{{ $input_num }}][]" id="netWeight_{{ $loop->index + 1 }}" placeholder="Net Weight" value="{{ $stock->net_weight }}"   oninput="calculate(this)"  min = "0" step = "any"  style="width:80px;">
                    </td>
                    <td>
                        <input type="number" class="tb_input" name="purity[{{ $input_num }}][]" id="purity_{{ $loop->index + 1 }}" placeholder="Purity" oninput="calculate(this)" value="{{ $stock->purity }}"   min = "0" step = "any"  style="width:80px;">
                    </td>
                    <td>
                        <input type="number" class="tb_input" name="wastage[{{ $input_num }}][]" id="wastage_{{ $loop->index + 1 }}" placeholder="Wastage" oninput="calculate(this)" value="{{ $stock->wastage }}"   min = "0" step = "any" style="width:80px;">
                    </td>
                    <td>
                        <input type="number" class="tb_input" name="fine_purity[{{ $input_num }}][]" id="finePurity_{{ $loop->index + 1 }}" placeholder="Fine Purity" readonly value="{{ $stock->fine_purity }}"   min = "0" step = "any" style="width:80px;">
                    </td>
                    <td>
                        <input type="number" class="tb_input" name="fine_weight[{{ $input_num }}][]" id="fineWeight_{{ $loop->index + 1 }}" placeholder="Fine Weight" readonly value="{{ $stock->fine_weight }}"   min = "0" step = "any" style="width:80px;" >
                    </td>
                    <td>
                        <input type="number" class="tb_input" name="labour_charge[{{ $input_num }}][]" id="labourCharge_{{ $loop->index + 1 }}" placeholder="Labour Charge" oninput="calculate(this)" value="{{ $stock->labour_charge }}"   min = "0" step = "any"  style="width:80px;">
                    </td>
                    <td>
                        <input type="number" class="tb_input" name="amount[{{ $input_num }}][]" id="amount_{{ $loop->index + 1 }}" placeholder="Amount" readonly value="{{ $stock->amount }}"   min = "0" step = "any" style="width:80px;">
                    </td>
                </tr>
                @php 
                    $num++ ;
