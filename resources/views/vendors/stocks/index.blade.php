...
@section('content')
    ...
    @php
        $metal_name = ucfirst($metal);
        //$data = component_array('breadcrumb' , "{$metal_name} Stock",[['title' => 'Stocks']] ) ;

    @endphp

    {{--<x-page-component :data=$data />--}}
@php 
$anchor = ['<a href="'.route('stocks.create').'" class="btn btn-sm btn-outline-primary"><i class="fa fa-plus"></i> New</a>','<a href="'.route('stocks.home').'" class="btn btn-sm btn-outline-info"><i class="fa fa-object-group"></i> Stocks</a>'];
$path = ["Stocks"=>route('stocks.home')];
$data = new_component_array('newbreadcrumb',"{$metal_name} Stocks",$path) 
@endphp 
<x-new-bread-crumb :data=$data :anchor=$anchor/> 
	...
	{{--<div class="card-header">
        <a href="{{ route('stocks.create') }}" class="btn btn-sm btn-primary"><li class="fa fa-plus"></li> Add New</a>
      </div>--}}
    <div class="card-body">
    ...
    <form action="{{ route('stocks.place') }}" method="post" id="counter_place_form">
        ...
        <div class="row">
			...
			@endphp
			<input type="hidden" name="metal" id="metal" value="{{ $metal }}">
...
@section('javascript')
    ...
    <script>
      ...
      const loading_tr = '<tr><td colspan="{{  ($metal !='artificial')?11:9 }}" class="text-center"><span class="p-1" style="background:lightgray;"><li class="fa fa-spinner fa-spin"></li> Loading Content..</span></td></tr>';
		...
		$(document).on('change','.tag_check',function(){
		  ...
		  $("#print_tag").click(function(e){
			...
			if($('[name="tagprint"]').val()!=""){
			  ...
			  var stock = $('[name="metal"]').val();
			  var code = $('[name="tagprint"]').serialize();
			  //$('#tag_code_modal').modal();
			  $("#tag_code_modal").find('.modal-body').load(path,data+"&stock="+stock+"&"+code,function(){
				$('#tag_code_modal').modal();
			  });
			}else{
