...
@endphp
<script>
$(document).on('change.gold','.caret',function(){
    const index = itemcheck($(this));
    const caret = $(this).val()??false;
    if(!caret){
        toastr.error('Please Select The Caret !');
    }else{
        const tunch = (caret/24 * 100);
        $(document).find('.tunch').eq(index).val(Math.round(tunch));
    }
    calculatesum(index);
});

$(document).on('input.gold','.gross',function(){
    const index = itemcheck($(this));
...
$(document).on('input.gold','.less',function(){
    const index = itemcheck($(this));
...
$(document).on('blur.gold','.tunch',function(){
    const index = itemcheck($(this));
    ...
    if(!tunch){
        ...
        if(!caret){
            toastr.error("Caret required to Create the Tunch !");
             ...
             $(document).find('.tunch').eq(index).val(Math.round(100/24 * caret));
        }
    }else{
        if(tunch > 100){
            $(document).find('.tunch').eq(index).val($(document).find('.tunch').val().slice(0,-1));
...
$(document).on('input.gold','.tunch',function(){
    const index = itemcheck($(this));
    const tunch = $(document).find('.tunch').eq(index).val()??false;
    if(!tunch){
		const caret = $(document).find('.caret').eq(index).val()??false;
...
/*$(document).on('blur.gold','.tunch',function(){
    ...
    if(!tunch){
        ...
        if(caret){
            let nw_t =  Math.round((100/24 * caret));
			nw_t = (+nw_t==75)?+nw_t + +1:nw_t;
            if(nw_t != tunch){
...
$(document).on('input.gold','.wstg',function(){
    const index = itemcheck($(this));
    const net = $(document).find('.net').eq(index);
...
$(document).on('input.gold','.chrg',function(){
    const index = itemcheck($(this));
...
$(document).on('input.gold','.rate',function(){
    const index = itemcheck($(this));
    const net = $(document).find('.net').eq(index).val()??false;
    if(!net){
        toastr.error("Net Missing Re-Enter the Gross !");
...
$(document).on('input.gold','.lbr',function(){
    const index = itemcheck($(this));
    const net = $(document).find('.net').eq(index).val()??false;
    if(!net){
        toastr.error("Net Missing Re-Enter the Gross !");
...
$(document).on('change.gold','.lbrunit',function(){
    const index = itemcheck($(this));
...
$(document).on('input.gold','.disc',function(){
    const index = itemcheck($(this));
    const net = $(document).find('.net').eq(index).val()??false;
    if(!net){
        toastr.error("Net Missing Re-Enter the Gross !");
...
$(document).on('change.gold','.discunit',function(){
    const index = itemcheck($(this));
...
$(document).on('input.gold','.other',function(){
    const index = itemcheck($(this));
    const net = $(document).find('.net').eq(index).val()??false;
    if(!net){
        toastr.error("Net Missing Re-Enter the Gross !");
...
$(document).on('input.gold','.ttl',function(){
    calculatesum();
});
...
function calculatetunchfine(index){
    ...
    if(caret){
        tounch = 100/24 * caret;
		tounch = (tounch==75)?tounch+1:tounch;
        $(document).find('.tunch').eq(index).val(Math.ceil(tounch));
    }
	var fine = net??0;
    if(tounch && wastage){
		const wastage_val = Math.round(+tounch + +wastage);
		//alert(+tounch + +wastage);
    ...
    const caret = $(document).find('.caret').eq(index).val()??false;
    const wastage = $(document).find('.wstg').eq(index).val()??0;
    const tounch = $(document).find('.tunch').eq(index).val()??0;
	if(tounch && net){
        const diff = +net * (100-(+tounch + +wastage))/100;
        $(document).find('.fine').eq(index).val((+net - diff).toFixed(3));
    }
}
...
function calculatesum(index){
    ...
    $(document).find('#list_item').val(item_count||0)
