            ...
            <table class="table table-bordered table_theme">
                ...
                <tbody class="billing item_tbody" id="item_data_area">
                    @for($i=0;$i<=5;$i++)
                        <tr class="item_tr" id="item_tr_{{ $i }}">
                            ...
                            <td>
                               <!-- <input type="text" class="form-control no-border chrg item_input" name="chrg[]" id="chrg_{{ $i }}" readonly>-->
                ...
                <tfoot>
                    <tr>
                        ...
                        <td colspan="2"></td>
                        <td>
                            <input type="text" class="form-control no-border no-hover item_input text-center" readonly value="" id="list_fine">
                        </td>
                        <td>
                            <input type="text" class="form-control no-border no-hover item_input text-center" readonly value="" id="list_chrg">
                        </td>
                        <td colspan="2"></td>
                        <td>
...
<script class="script_section" id="gold_script">    
    

    $(document).on('click.gold',".add_assos_element",function(e){
        e.preventDefault();
        ...
        if(!fine){
            toastr.error("Item Fine required Check the Caret or Gross/Net!");
            $(document).find('.fine').eq(index).trigger('input');
        }else{
    ...
    $(document).on('focus.gold','.item',function(){
    ...
    $(document).on('input.gold','.item',function(){
        let input = $(this);
        ...
        if(input.val()!=""){
            ...
            $.get("{{ route('stock.find.item') }}?stock=gold&entry="+$('#entry_type').val(),"keyword="+$(this).val(),function(response){
                let lis = '';
                if(response.items){
    ...
    $(document).on('click.gold','.get_item',function(){
...
@include('vendors.stocks.newpage.itemforms.js.goldcalculate')
