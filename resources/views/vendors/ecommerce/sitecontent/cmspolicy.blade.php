...
//$data = component_array('breadcrumb' , 'Privacy Policy',[['title' => 'Pages']] ) ;

@endphp

{{--<x-page-component :data=$data />--}}
@php $data = new_component_array('newbreadcrumb',"Ecomm Privacy Policy") @endphp 
  ...
  <div class="row justify-content-center">
    ...
    <div class="card-body">
       ...
       <div class="col-md-12 current_status">
		@php 
			$status = $existpolicy->policy_status??false;
		@endphp
       <form action="{{ route("privacypolicy") }}" method="post" role="form" id = "term_content_form" class="{{ (@$existpolicy->policy_status==0)?'deactive':'' }}">

        @csrf

          <div class="form-group col-md-12">
               <label for="intro" class="required_label col-12">Privacy Policy 
			   <a href="{{ route('privacypolicy.status') }}" class="switch {{ (@$existpolicy->policy_status==0)?'off':'on' }}"></a></label>
               <textarea class="form-control ckeditor" id = "info" >{{ @$existpolicy->policy_content }}</textarea>
          </div>
