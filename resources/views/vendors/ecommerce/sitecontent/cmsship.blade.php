...
@section('content')
	...
	//$data = component_array('breadcrumb' , 'Shiping Policy',[['title' => 'Pages']] ) ;

    @endphp

    {{--<x-page-component :data=$data />--}}
@php $data = new_component_array('newbreadcrumb',"Ecomm Shiping Policy") @endphp 
...
<div class="container-fluid">
  ...
  <div class="row justify-content-center">
    ...
    <div class="card-body">
       ...
       <form action="{{ route("shiping") }}" method="post" role="form" id = "term_content_form" class="{{ (@$existdesc->policy_status==0)?'deactive':'' }}">

        @csrf

          <div class="form-group col-md-12">
               <label for="intro" class="required_label col-12">Shiping Policy <a href="{{ route('shiping.status') }}" class="switch {{ (@$existdesc->policy_status==0)?'off':'on' }}"></a></label>
               <textarea class="form-control ckeditor" id = "info" >{{ @$existdesc->policy_content }}</textarea>
          </div>
