...
@section('content')
	...
	//$data = component_array('breadcrumb' , 'Desclaimer',[['title' => 'Pages']] ) ;

    @endphp

    {{--<x-page-component :data=$data />--}}
@php $data = new_component_array('newbreadcrumb',"Ecomm Desclaimer") @endphp 
...
<div class="container-fluid">
  ...
  <div class="row justify-content-center">
    ...
    <div class="card-body">
       ...
       <form action="{{ route("desclaimer") }}" method="post" role="form" id = "term_content_form" class="{{ (@$existdesc->desc_status==0)?'deactive':'' }}">

        @csrf

          <div class="form-group col-md-12">
               <label for="intro" class="required_label col-12">Desclaimer <a href="{{ route('desclaimer.status') }}" class="switch {{ (@$existdesc->desc_status==0)?'off':'on' }}"></a></label>
               <textarea class="form-control ckeditor" id = "info" >{{ @$existdesc->desc_content }}</textarea>
          </div>
