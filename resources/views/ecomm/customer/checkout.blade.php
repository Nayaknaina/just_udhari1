...
<div class="container-fluid bg-secondary breadcrumb-section">
    <div class="d-flex flex-column align-items-center justify-content-center  px-2" >
        <div class="customer_page_head col-md-4 row text-center px-1" >
            ...
            <h1 class="font-weight-semi-bold text-uppercase  my-2" style="margin:auto;">Checkout</h1>
        ...
        <div class=" row">
          <div class="col-lg-8 p-0">
            ...
            <div class="mb-4 col-12  shipping_address" id="current_address" style="display:{{ @$old_style }};" >
              ...
              <div class="row">
                ...
                <div class="col-md-6 form-group">
				  ...
				  @php  
                      $district_data = (old('old_state'))?districts(old('old_state')):$districts
                  @endphp
                  <select class="form-control addr_dist" name="old_dist" id="old_dist" >
                    @if(count($district_data)>0)
                        <option value="">Select District</option>
                        @foreach($district_data as $di=>$district)
                          @php 
                            $choosed_dist = old('old_dist')??$customer->dist_id;
                            $selected_dist = ($district->code==$choosed_dist)?'selected':'';
                          @endphp
                          <option value="{{ $district->code}}" {{ $selected_dist }} >{{ $district->name }}</option>
                        @endforeach
                      @else
                ...
                <div class="col-md-6 p-0">
                  <div class="col-md-12 form-group">
                    <label>Tehsil</label>
                    <input class="form-control" type="text" name="old_teh" placeholder="Sub District / Tehsil" value="{{ old("old_teh")??$customer->teh_name }}">
                    @error("old_teh")
                      <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
                  <div class="col-md-12 form-group">
                    <label>Area</label>
                    <input class="form-control" type="text" name="old_aera" placeholder="Area" value="{{ old("old_aera")??$customer->area_name }}">
                    @error("old_aera")
                      <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
                  <div class="col-md-12 form-group">
                    ...
                    <input class="form-control" type="text" name="old_pin" placeholder="123456" value="{{ old("old_pin")??$customer->pin_code }}">
                    @error("old_pin")
                      <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
            ...
            <div class="mb-4 col-12  shipping_address" id="new_address" style="display:{{ @$new_style }}" >
              ...
              <div class="row">
                <div class="col-md-12 form-group">
                  <label>Address Line 1</label>
                  <input class="form-control" type="text" name="new_addr_one" placeholder="Address Line 1" value="{{ old("new_addr_one")??@$shiping->custo_addr_one }}">
                  @error("new_addr_one")
                    <small class="text-danger">{{ $message }}</small>
                  @enderror
                </div>
                <div class="col-md-12 form-group">
                  <label>Address Line 2</label>
                  <input class="form-control" type="text" name="new_addr_two"  placeholder="Address Line 2" value="{{ old("new_addr_two")??@$shiping->custo_addr_two }}">
                  @error("new_addr_two")
                    <small class="text-danger">{{ $message }}</small>
                  @enderror
                </div>
                <div class="col-md-6 form-group">
                  ...
                  <select class="form-control addr_state" name="new_state" id="new_state" data-target="new_dist">
					...
					@foreach($states as $si=>$state)
					  @php 
						$choosed_state = old("new_state")??@$shiping->state_id;
						$selected_state = ($state->code==$choosed_state)?'selected':'';
					  @endphp
					  <option value="{{ $state->code}}" {{ $selected_state }}>{{ $state->name }}</option>
					@endforeach
                ...
                <div class="col-md-6 form-group">
                  <label class="required">City</label>
				  @php  
                      $from_state  = old('new_state')??@$shiping->state_id;
                      $new_district_data = ($from_state!="")?districts($from_state):[];
                  @endphp
                  <select class="form-control addr_dist" name="new_dist" id="new_dist"  >
                    @if(count($new_district_data)>0)
                        <option value="">Select District</option>
                        @foreach($new_district_data as $di=>$district)
                          @php 
                            $choosed_dist = old('new_dist')??@$shiping->dist_id;
                            $selected_dist = ($district->code==$choosed_dist)?'selected':'';
                          @endphp
                          <option value="{{ $district->code}}" {{ $selected_dist }} >{{ $district->name }}</option>
                        @endforeach
                ...
                <div class="col-md-6 p-0">
                  <div class="col-md-12 form-group">
                    <label>Tehsil</label>
                    <input class="form-control" type="text" name="new_teh" placeholder="Sub District / Tehsil" value="{{ old("new_teh")??@$shiping->teh_name }}">
                    @error("new_teh")
                      <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
                  <div class="col-md-12 form-group">
                    <label>Area</label>
                    <input class="form-control" type="text" name="new_aera" placeholder="Area" value="{{ old("new_aera")??@$shiping->area_name }}">
                    @error("new_aera")
                      <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
                  <div class="col-md-12 form-group">
                    <label>ZIP Code</label>
                    <input class="form-control" name="new_pin" type="text" placeholder="123456" value="{{ old("new_pin")??@$shiping->pin_code }}">
                    @error("new_pin")
                      <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
                ...
                <div class="col-md-6 p-0">
                  <div class="col-md-12 form-group">
                    <label class="required">Full Address</label>
                    <textarea name="new_address" class="form-control" placeholder="Full Address" rows="8"  id="">{{ old("new_address")??@$shiping->custo_address }}</textarea>
                    @error("new_address")
                      <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
          ...
          <div class="col-lg-4">
              ...
              <div class="card-body p-2">
                ...
                @if(count($detail->toArray())>0)
                  ...
                  <div class="d-flex justify-content-between">
                     @php 
                      $cost = round($dtl->quantity*$dtl->curr_cost,2)+($dtl->quantity*$dtl->product->strike_rate);
                      $total+= $cost; 
                    @endphp
            ...
            <div class="card border-secondary mb-5">
              <div class="card-header bg-secondary border-0">
                <h4 class="font-weight-semi-bold m-0 text-white">Payment</h4>
              </div>
              <div class="card-body">
                @php 
                ...
                @endphp
                @if($gateway_count>0)
                  @foreach($gateways as $gi=>$gateway)
                  <div class="form-group">
                    <div class="custom-control custom-radio">
                      <input type="radio" class="custom-control-input" name="gateway" value="{{ $gateway->id }}" id="{{ $gateway->id }}" {{ ($gateway_count==1)?'checked':'' }} >
                      <label class="custom-control-label" for="{{ $gateway->id }}">{{ ucfirst(str_replace("_"," ",$gateway->gateway_name)) }}</label>
                    </div>
                  </div>
                  @endforeach
                @endif
...
/*$("#order_form").submit(function(e){
	...
	$.post($(this).attr('action'),$(this).serialize(),function(response){
		const newWin = window.open('', "_system"); // open blank window
		newWin.document.open();
		newWin.document.write(response.html);    // write your form HTML
