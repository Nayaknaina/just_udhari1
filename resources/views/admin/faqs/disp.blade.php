...
<table id="CsTable" class="table table-striped table-bordered text-nowrap align-middle dataTable">
    ...
    @foreach($faqs as $key=>$row)
        <tr>
            ...
            <td>
                ...
                <a class="btn btn-outline-success" href="{{route('faqs.edit', $row->id)}}">Edit</a>
                <form action="{{ route('faqs.destroy',$row->id) }}" method="POST">
                @csrf
                @method('DELETE')
  ...
  @include('layouts.theme.datatable.pagination', ['paginator' => $faqs,'type'=>1])
