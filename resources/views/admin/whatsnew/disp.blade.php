...
<table id="CsTable" class="table table-striped table-bordered text-nowrap align-middle dataTable">
    ...
    @foreach($whatsnew as $key=>$row)
        <tr>
            <td>{{ $whatsnew->firstItem() + $key }}</td>
            <td>{{$row->title }}</td>
            <td > <img src="{{ asset('assets/images/whatsnew/'.$row->image) }}" class=" tb_img img-fluid img-thumbnail"> </td>
            <td>
                <div class="d-flex justify-content-betw een">
                <a class="btn btn-outline-success" href="{{route('whatsnew.edit', $row->id)}}">Edit</a>
                <form action="{{ route('whatsnew.destroy',$row->id) }}" method="POST">
                @csrf
                @method('DELETE')
  ...
  @include('layouts.theme.datatable.pagination', ['paginator' => $whatsnew,'type'=>1])
